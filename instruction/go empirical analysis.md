方法概述：在github仓库跑出编译日志后 ，根据每个未知分类（unknown）的情况，在本地构造小型`demo`示例（`go/demo/demo.go`中），结合调试，修改escape文件夹下的go文件。（判断逻辑集中在`solve.go`的`countAll`函数中）

调试：可通过`go/.vscode/launch.json`进行调试，program和GOROOT路径需要根据实际更改。（多线程函数不可调试）
...
③脚本收集：统计脚本位于`go/countAll/countAll.go`中，根据实际目录进行修改，运行即可。
